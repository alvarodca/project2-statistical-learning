...
This interpretability may be hard to obtain as some models are 'black boxes' and it is complicated to see how they arrived at their conclusions. We will use SHAP values (SHapley Additive exPlanations) which measures how much each feature contributes to the prediction, we will use it for models which do not support categorical data as if not we get an error, which is due to training the model with categorical data. Moreover, we will make the plot ROC curves which we will use in the future for comparing models.
...
With this logic, we will select the best performing models we have tried, these were: Kernel SVM, Ordinal Logistic Regression, Regularized Discriminant Analysis and XGBoost. We will train them with the optimal hyper-parameters found before.
...
for (i in seq_along(folds)) {
  ...
  pred_olr <- convert_to_numeric(predict(final_olr_cv, test_data_cv))
  ...
  average_prediction <- (pred_rda + pred_ksvm + pred_olr + pred_xgboost) / 4
...
ensemble_predict <- function(test_data) {
  ...
  pred_olr <- convert_to_numeric(predict(final_olr, test_data))
