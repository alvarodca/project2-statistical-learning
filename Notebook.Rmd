...
required_libraries <- c("skimr", "ggplot2","dplyr", "VIM", "dbscan", "RColorBrewer", "isotree" ,"e1071", "bestNormalize","caret", "GGally", "corrplot", "rpart", "rpart.plot", "MASS","biotools", "klaR", "iml", "fastshap","randomForest", "gbm", "xgboost","reshape2", "pdp","pROC")
...
ggplot(pca_data, aes(x = PC1, y = PC2, color = Predicted_Class_kNN, shape=SalePrice_Category)) +
  ...
  theme_minimal() +
  scale_color_manual(values = rainbow(length(unique(pca_data$Predicted_Class_kNN))))  # Customized colors for predicted classes
```
...
```{r}
# Defining a prediction wrapper for probabilities (if required)
...
We can see that the value obtained for accuracy is around 0.65 and around 0.55 for the kappa value which are somewhat decent values and better than doing it by random chance but we could improve this values for sure.
...
The plot shows how each feature contributes to the model output. Wider distributions indicate more variability in feature impact. The center of the distribution around zero suggests a neutral effect. A violin with mean around 0 has almost no impact as its shap values are near zero. If they are wide and far from zero means that it varies but with significant SHAP values which in this case ExterQual can have a negative SHAP for low price houses and high SHAP for high price houses.
...
The plot shows how each feature contributes to the model output. Wider distributions indicate more variability in feature impact. A violin with mean around 0 has almost no impact as its shap values are near zero. If they are wide and far from zero means that it varies but with significant SHAP value. MSSubClass has no impact on this model whereas ExterQual is clearly the most significant in this case.
...
We see that there are 5 different link models which use different methods and functions to determine the cumulative properties of the response variables. Each of these methods behave better depending on the distribution of data, in our case we expect Probit or Logit to be the mest method as it assumes a normal distribution. However we see that all methods return really similar values which means they perform really well.
...
The best accuracy obtained was of 0.7 with a kappa of 0.63 which is good compared to other models but there is not a really big improvement.
...
We observe a very drastical improvement with a 74% accuracy, the highest yet. With this we prove that Gradient Boosting is still one of the most robust methods, but we just have to take the right approach.
...
## 4.1 Building the final classifier
...
Firstly we will train the RDA model.
...
# Defining a tuning grid for both gamma and lambda
tune_grid <- expand.grid(gamma = 0, 
                         lambda = 0.9)  

# Training the regularized QDA model with cross-validation and tuning
final_rda <- train(SalePrice_Category ~ ., data = data_enc, method = "rda", 
                   trControl = train_control, tuneGrid = tune_grid)

# Displaying the results
print(final_rda$results)
```

We obtain around a 70% accuracy as expected.

We now will train the Ordinal Logistic Regression model.
...
The ensemble predict model returned an accuracy of 0.85. We will now create some graphs to interpret this model and obtain insights about the data.
...
As seen with most of our previously trained models the models give a big importance to both Area and Quality which makes sense as they are two categories people take into account when buying houses.

Moreover, BldgType and MSSubClass dont seem to have a really high significance as the rest of models.

Based on this graph, we will make a partial dependence plot on Overall Qual as it is clearly the most important variable when classifying, to see how it affects each class.

```{r}
# Compute Partial Dependence for each model
pdp_rda <- partial(
  final_rda,
...
# Combine the Partial Dependence results into a single data frame
ensemble_pd <- data.frame(
  Overall_Qual = pdp_rda$OverallQual,  # Grid values (assumes same grid across models)
  RDA = pdp_rda$yhat,
  XGBoost = pdp_xgboost$yhat
)

# Compute the ensemble's averaged Partial Dependence
ensemble_pd$Ensemble <- rowMeans(ensemble_pd[, c("RDA", "XGBoost")])

# Plot the Ensemble PDP (single line)
ggplot(ensemble_pd, aes(x = Overall_Qual, y = Ensemble)) +
  geom_line(color = "steelblue", size = 1.2) +
  labs(
    title = "Partial Dependence Plot for Overall Quality (Ensemble)",
    x = "Overall Quality",
...
The Y axis represents the Predicted Probability of the target variable Sale Price actually happening. The x axis shows the values of the Overall Quality which are between -2.5 and 5 (they have been scaled). This clearly shows a negative relation between both of these variables, the higher the Quality the lower the probability. When the slope seems to flatten which here happens close to 0 just shows that the feature is not as important for these values.

# For reproducibility purposes

set.seed(123)

Creating the ROC curves based on the test

```{r}
# Function to calculate predicted probabilities for the training set
ensemble_predict_probabilities_train <- function(train_data) {
  train_labels <- train_data[, 25]  # Assuming labels are in column 25
  train_data <- train_data[, -25]  # Exclude the label column for prediction
  
  # Generate probability predictions for each model (RDA, SVM, XGBoost)
  prob_rda <- predict(final_rda, train_data, type = "prob")
  prob_xgboost <- predict(final_xgboost, train_data, type = "prob")
  
  # Calculate ensemble probabilities as the mean of probabilities from each model
  ensemble_probs <- (prob_rda + prob_xgboost) / 2
  
  return(list(probs = ensemble_probs, labels = train_labels))
}

# Get ensemble probabilities and true labels for the training set
ensemble_results_train <- ensemble_predict_probabilities_train(data_enc)
ensemble_probs_train <- ensemble_results_train$probs
true_labels_train <- ensemble_results_train$labels

# Create a list to store ROC curves for each class (One-vs-Rest)
roc_list_ensemble_train <- list()

# Get the levels of the true labels (class names)
class_levels_train <- levels(true_labels_train)

# Calculate ROC curve for each class using One-vs-Rest approach
for (i in 1:length(class_levels_train)) {
  # Treat the current class as positive (1) and all others as negative (0)
  binary_true_labels <- ifelse(true_labels_train == class_levels_train[i], 1, 0)
  
  # Calculate ROC curve for the current class
  roc_list_ensemble_train[[i]] <- roc(binary_true_labels, ensemble_probs_train[, i], quiet = TRUE)
}

# Plot ROC curves using ggroc
roc_plot_ensemble_train <- ggroc(roc_list_ensemble_train, aes = "color") +
  ggtitle("One-vs-Rest ROC Curves for Ensemble Model (Training Set)") +
  xlab("False Positive Rate") +
  ylab("True Positive Rate") +
  theme_minimal()

# Print the ROC curves and the plot
print(roc_list_ensemble_train)
print(roc_plot_ensemble_train)
