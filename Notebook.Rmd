...
required_libraries <- c("skimr", "ggplot2","dplyr", "VIM", "dbscan", "RColorBrewer", "isotree","Rlof")
...
```{r}
# Apply DBSCAN
scaled_num_data <- as.data.frame(scale(numeric_data))

# Calculate k-distances (e.g., 4th nearest neighbor)
kNNdistplot(scaled_num_data, k = 4)
...
model <- dbscan(scaled_num_data, eps = 6, minPts = 4)

pca <- prcomp(scaled_num_data, center = TRUE)
...
```{r}
isolation_model <- isolation.forest(numeric_data, ndim = 1, ntrees = 100)
# We could change both ndim and ntrees for different results, but these results are good

outlier_score <- predict(isolation_model, numeric_data, type = "score")
...
threshold <- quantile(outlier_score, 0.99)
