...
predictors <- full_data[,-which(names(full_data) == "SalePrice")]
target_variable <- full_data$SalePrice
...
rfe <- rfe(predictors, target_variable, sizes = seq(10, 35, by = 5), rfeControl = control)
...
selected_features <- predictors(rfe)
